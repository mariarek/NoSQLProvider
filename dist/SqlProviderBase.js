        ...
        return this._getMetadata(trans).then(function (fullMeta) {
            ...
            return trans.runQuery('SELECT type, name, tbl_name, sql from sqlite_master', [])
                ...
                var deleteFromMeta = function (metasToDelete) {
                    ...
                    var placeholder = '?';
                    for (var i = 1; i < metasToDelete.length; i++) {
                        placeholder += ',?';
                    }
                ...
                return SyncTasks.all(dropQueries).then(function () {
                    ...
                    _.each(_this._schema.stores, function (storeSchema) {
                        ...
                        var indexColumns = _.map(columnBasedIndices, function (index) { return 'nsp_i_' + index.name + ' TEXT'; });
                        ...
                        var newIndices = _.filter(storeSchema.indexes, function (index) {
                            var indexIdentifier = getIndexIdentifier(storeSchema, index);
                            return !_.some(currentIndexMetas, function (meta) { return meta.key === indexIdentifier; });
                        });
                        // find indices in the meta that do not exist in the new schema
                        var removedIndexMetas = _.filter(currentIndexMetas, function (meta) {
                            return !_.some(storeSchema.indexes, function (newIndex) { return getIndexIdentifier(storeSchema, newIndex) === meta.key; });
                        });
                        var _a = _.partition(removedIndexMetas, function (meta) { return indexUsesSeparateTable(meta.index, _this._supportsFTS3); }), removedTableIndexMetas = _a[0], removedColumnIndexMetas = _a[1];
                        ...
                        var adddNewColumns = tableExists && !doSqlInPlaceMigration && newNoBackfillIndices.length > 0;
                        if (!tableExists) {
                            ...
                            var indexDroppers = _.concat(dropColumnIndices(), dropIndexTables(indexTables[storeSchema.name]));
                            ...
                            tableQueries.push(SyncTasks.all(indexDroppers)
                            ...
                            var sqlInPlaceMigrator = function () {
                                return trans.runQuery('INSERT INTO ' + storeSchema.name +
                                    ' SELECT ' + ['nsp_pk', 'nsp_data'].concat(indexColumns).join(', ') +
                                    ' FROM temp_' + storeSchema.name);
                            };
                            var indexDroppers = dropIndexTables(removedTableIndexMetas.map(function (meta) { return getIndexIdentifier(storeSchema, meta.index); }));
                            if (removedIndexMetas.length > 0) {
                                indexDroppers.push(deleteFromMeta(removedIndexMetas));
                            }
                            tableQueries.push(SyncTasks.all(indexDroppers)
                            ...
                            tableQueries.push(SyncTasks.all(indexDroppers)
                                .then(columnAdder)
                                .then(function () { return indexMaker(newNoBackfillIndices); }));
                        }
                    });
